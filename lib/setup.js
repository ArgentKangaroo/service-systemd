...
setup.parse = function (settings) {
  ...
  if(settings.raw){
    settings.raws = ''
    for (const idx in settings.raw) {
      settings.raws += settings.raw[idx] + '\n'
    }
  }
  ...
  let _start = tools.string.template(_templates.engines[settings.engine].start, settings, true)
  let _stop = tools.string.template(_templates.engines[settings.engine].stop, settings, true)
  let _logrotate = settings.logrotate ? tools.string.template(_templates.logrotate, settings, true) : ''
